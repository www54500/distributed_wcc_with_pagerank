...
public class WccMasterCompute extends DefaultMasterCompute {
  ...
  public static final int MERGE_COMMUNITIES_INITIALIZATION = 15;
  public static final int MERGE_COMMUNITIES_FIND_TOP = 16;
  public static final int FIND_SECONDARY_COMMUNITIES = 17;
  public static final int MERGE_SECONDARY_COMMUNITIES = 18;
  public static final int COLLECTING_PRIMARY = 19;
  public static final int FIND_PRIMARY_COMMUNITIES = 20;
  public static final int MERGE_PRIMARY_COMMUNITIES = 21;
  ...
  public static final String EPSILON_CONF_OPT = "pr.epsilon";
  ...
  public void compute() {
    ...
    if (maxDegree != Long.MIN_VALUE && maxDegree != 0) epsilon = (epsilon == 0.0) ? 1.0 / ((double) maxDegree) : epsilon;
    ...
    switch (nextPhase) {
      ...
      case COMPUTE_WCC:
        ...
        if (retriesLeft == 0) {
          ...
          setAggregatedValue(LIMIT_ROUND, new IntWritable(10));// at least 10 round
      ...
      case MERGE_COMMUNITIES_FIND_TOP:
        if (previousPhase == MERGE_COMMUNITIES_INITIALIZATION) {
          ...
          int topCommunity = 0;
          // choose the community size <= 3 be the secondarys
      ...
      case MERGE_SECONDARY_COMMUNITIES:
        ...
        // reset top community map at same time
        for (Map.Entry<Writable, Writable> e : topComm.entrySet()) {
      ...
      case FIND_PRIMARY_COMMUNITIES:
        ...
        // here need to save the previous topComm, since this topcomm is used to store the primarys which may be merged
      ...
      case MERGE_PRIMARY_COMMUNITIES:
        ...
        // NOTE! the community named "secondary" below in this case is also the primary community, so need to remove from the topcomm
        ...
        topComm = (MapWritable) getAggregatedValue(PREVIOUS_TOP_COMMUNITIES); // this map is the real top community map
        cutMap = (MapWritable) getAggregatedValue(CUT);
        volumeMap = (MapWritable) getAggregatedValue(COMMUNITY_VOLUME);
        HashMap<Integer, double[]> communityMapWithCond = new HashMap<Integer, double[]>(); // a map store a community and its conductance
          ...
          for (int secondary : secondarys) {
            ...
            if (mergedConductance < primaryConductance && mergedConductance < secondaryConductance) { // two communities can merge
              if (mergedConductance < lowestConduct) { // every primary only merge with the community had lowest conductance
                lowestConduct = mergedConductance;
        ...
        // check whether there are some primarys want to merge with the same secondary
        // If there are, merge with the one with lower conductance
        ...
        // put the merged result to the map
        for (Map.Entry<Integer, double[]> e : communityMapWithCond.entrySet()) {
          IntWritable commWritable = new IntWritable(e.getKey());
          double[] arr = e.getValue();
          if (arr.length == 0) { continue; }
  ...
  public void initialize() throws InstantiationException, IllegalAccessException {
    ...
    String epsilonConf = getConf().get(EPSILON_CONF_OPT);
    epsilon = (epsilonConf != null) ? 
        Double.parseDouble(epsilonConf.trim()) : 0.0;
