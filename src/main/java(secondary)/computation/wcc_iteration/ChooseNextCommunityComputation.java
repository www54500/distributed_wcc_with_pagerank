...
public class ChooseNextCommunityComputation extends AbstractComputation<
  ...
  public void compute(
    ...
    if (oldCommunity != newCommunity) {
      sendMessageToAllEdges(vertex, new TransferMessage(vertex.getId().get(), newCommunity));
    }         
  }
  ...
  private boolean ifIsolatedCommunity(Vertex<IntWritable, WccVertexData, NullWritable> vertex) {
    //boolean ifIsolated = false;
    WccVertexData vData = vertex.getValue();
    IntWritable myCommunity = vertex.getId();
    MapWritable ncm = vData.getNeighborCommunityMap();
    ...
    candidates.remove(new IntWritable(vData.getCommunity()));
    return candidates;*/
    return !ncm.containsValue(myCommunity);
  }
}
